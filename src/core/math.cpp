...
const Transform Transform::identity = { vec3(0.f, 0.f, 0.f), quat(0.f, 0.f, 0.f, 1.f), vec3(1.f, 1.f, 1.f) };
...
mat2 operator*(const mat2& a, float b) { mat2 result; for (u32 i = 0; i < 4; ++i) { result.m[i] = a.m[i] * b; } return result; }
mat3 operator*(const mat3& a, float b) { mat3 result; for (u32 i = 0; i < 9; ++i) { result.m[i] = a.m[i] * b; } return result; }

#if ROW_MAJOR
mat4 operator*(const mat4& a, float b) { mat4 result; for (u32 i = 0; i < 4; ++i) { result.rows[i] = a.rows[i] * b; } return result; }
#else
mat4 operator*(const mat4& a, float b) { mat4 result; for (u32 i = 0; i < 4; ++i) { result.cols[i] = a.cols[i] * b; } return result; }
#endif
...
{
	float sqLength = squared_length(q.v);
	if (sqLength > 0.f)
	{
		angle = 2.f * acos(q.w);
...
mat4 invert_affine_matrix(const mat4& m)
{
	...
	vec3 invXAxis = xAxis / squared_length(xAxis);
	vec3 invYAxis = yAxis / squared_length(yAxis);
	vec3 invZAxis = zAxis / squared_length(zAxis);
...
static void jacobi_eigen_analysis(
	float& s11,
	float& s21, float& s22,
	float& s31, float& s32, float& s33,
...
SVD compute_svd(const mat3& A)
{
	...
	quat q;
	jacobi_eigen_analysis(ATA.m00, ATA.m10, ATA.m11, ATA.m20, ATA.m21, ATA.m22, q);
...
static void computeMinor(mat3& m, const mat3& from, u32 d)
{
	for (u32 y = 0; y < 3; ++y)
	{
		for (u32 x = 0; x < 3; ++x)
